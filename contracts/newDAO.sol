...
contract DAOProposal {
    ...
    uint256 public periodDuration; // periodDuration = 17280 seconds = 4.8 hours (5 periods per day)
    uint256 public votingPeriodLength; // votingPeriodLength = 35 periods (7 days)
    uint256 public deployedTime; // DAO contract deployed time
    ...
    struct Member {
        uint256 shares; // the voting shares assigned to this member
        uint256 loot; // the loot amount available to this member (combined with shares on ragequit(quitting the process))
    ...
    struct Proposal {
        ...
        uint256 startingTime; // the time in which voting can start for specific proposal
        uint256 yesVotes; // the total number of YES votes for proposal
        uint256 noVotes; // the total number of NO votes for proposal
        bool[6] flags; // [sponsored, processed, didPass, cancelled, memberAdd, memberKick]
        string details; // proposal details - could be IPFS hash, plaintext, or JSON
        mapping(address => Vote) votesByMember; // the votes on proposal by each member
        bool exists; // always true once a member has been created
    }

    enum Vote {
        Null, // default value is null, counted as absent
        Yes,
        No
    }
    ...
    // Only members can vote on a proposal.
    ...
    // ALLOWANCE FUNCTIONS
    // An allowance is given to a proposer when their proposal gets a majority vote and the voting period has 
    //expired.
    ...
    //Payout - the expected financial returns
    ...
    // OR kick members who don't want to contribute.
    function addMember(address newMemAddress, string memory details) public onlyMember {
        _SubmitMemberProposal(newMemAddress, details, 0); // 0 for adding a member
    }

    function kickMember(address memToKick, string memory details) public onlyMember {
        _SubmitMemberProposal(memToKick, details, 1);  // 1 for kicking a member
    }
    ...
    function submitProposal(uint256 paymentRequested, string memory details) public returns (uint256 proposalId) {
        ...
        require(applicant != address(0), "applicant cannot be 0 address");
    ...
    // Function for cancels a proposal if it has not been cancelled already.
    ...
    // Function which can be called when the proposal voting time has expired. To either act on the proposal 
    //or cancel if not a majority yes vote.
    function processProposal(uint256 proposalId) public onlyMember returns (bool) {
        ...
        if(prop.flags[4] == true) { // for Member add
        ...
        if(prop.flags[5] == true) { //for  Member kick
            if(prop.yesVotes > prop.noVotes) {
    ...
    function submitVote(uint256 proposalId, uint8 uintVote) public onlyMember {
        ...
        require(proposals[proposalId].exists, "This proposal does not exist.");
        require(uintVote < 3, "must be less than 3"); // [null,yes,no]  [0,1,2]
